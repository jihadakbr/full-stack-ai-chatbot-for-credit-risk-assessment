...
FEATURES_21 = [
    "SK_ID_CURR","EXT_SOURCE_1","GOODS_CREDIT_RATIO","EXT_SOURCE_3","EXT_SOURCE_2",
...
def prepare_features_21(raw: dict) -> dict:
    ...
    if all(k in raw for k in FEATURES_21):
        ...
        df["SK_ID_CURR"] = pd.to_numeric(df.get("SK_ID_CURR"), errors="coerce").astype("Int64")
    ...
    base_cols = [
        "SK_ID_CURR","EXT_SOURCE_1","AMT_GOODS_PRICE","AMT_CREDIT","EXT_SOURCE_3","EXT_SOURCE_2",
        "ORGANIZATION_TYPE","DAYS_BIRTH","DAYS_EMPLOYED","NAME_EDUCATION_TYPE","CODE_GENDER",
        "AMT_ANNUITY","FLAG_OWN_CAR","DAYS_ID_PUBLISH","AMT_INCOME_TOTAL","OWN_CAR_AGE","LIVINGAREA_MEDI",
        "DEF_30_CNT_SOCIAL_CIRCLE","FLAG_DOCUMENT_3","NAME_FAMILY_STATUS","NAME_INCOME_TYPE"
    ]
    for c in base_cols:
    ...
    if ("ORGANIZATION_TYPE_ENCODED" not in df.columns) or pd.isna(df.loc[0, "ORGANIZATION_TYPE_ENCODED"]):
    ...
    df['CODE_GENDER'] = df['CODE_GENDER'].fillna(MOST_FREQ_GENDER).replace('XNA', MOST_FREQ_GENDER)
    df["CODE_GENDER"] = df["CODE_GENDER"].apply(lambda x: 1 if str(x).upper() == "F" else 0)
    ...
    df = pd.get_dummies(df, columns=["NAME_FAMILY_STATUS","NAME_INCOME_TYPE"], dtype=int, drop_first=True)
