    ...
    def _prompt_for_current_step(self):
        prompts = {
            1: (
                "CODE_GENDER",
                "What is your gender?\n"
                + _format_options(GENDER_OPTIONS)
                + PROMPT_SUFFIX_PICK,
            ),
            2: (
                "AGE",
                "What is your age in years? (Two digits, e.g., 18, 30, 50)"
                + PROMPT_SUFFIX_NUMBER,
            ),
            3: (
                "NAME_FAMILY_STATUS",
                "What is your family status?\n"
                + _format_options(FAMILY_STATUS_OPTIONS)
                + PROMPT_SUFFIX_PICK,
            ),
            4: (
                "FLAG_OWN_CAR",
            ...
            6: (
                ...
                + _format_options(YES_NO)
                + PROMPT_SUFFIX_PICK,
            ),
            7: (
                "ORGANIZATION_TYPE",
                "Which best describes your organization type?\n"
                + _format_options(ORG_TYPES)
                + PROMPT_SUFFIX_PICK,
            ),
            8: (
                "DAYS_EMPLOYED",
                "How many days have you been employed at your current job? (Count from your start date up to today, e.g., 30, 45, 62, 100, 360)"
                + PROMPT_SUFFIX_NUMBER,
            ),
            9: (
                "NAME_EDUCATION_TYPE",
                "What is your highest education level?\n"
                + _format_options(EDU_OPTIONS)
                + PROMPT_SUFFIX_PICK,
            ),
            10: (
                "NAME_INCOME_TYPE",
                "What is your income type?\n"
                + _format_options(INCOME_TYPE_OPTIONS)
                + PROMPT_SUFFIX_PICK,
            ),
            11: (
                "AMT_GOODS_PRICE",
                "What is the price of the item you plan to purchase with this loan? (e.g., valid formats: 1500 or 1,500)"
                + PROMPT_SUFFIX_NUMBER,
            ),
            12: (
                "AMT_CREDIT",
                "What total loan amount are you applying for? (e.g., valid formats: 1500 or 1,500)"
                + PROMPT_SUFFIX_NUMBER,
            ),
            13: (
                "AMT_ANNUITY",
                "What is the expected monthly installment? (e.g., valid formats: 1500 or 1,500)"
                + PROMPT_SUFFIX_NUMBER,
            ),
            14: (
                "AMT_INCOME_TOTAL",
                "What is your total monthly income? (e.g., valid formats: 1500 or 1,500)"
                + PROMPT_SUFFIX_NUMBER,
            ),
            15: (
                "DAYS_ID_PUBLISH",
        ...
        try:
            if self.step == 1:
                ...
                if not sel:
                    return (
                        False,
                        "Please reply with a number 1–3 for gender:\n"
                        + _format_options(GENDER_OPTIONS)
                        + PROMPT_SUFFIX_PICK,
                    )
                self.answers["CODE_GENDER"] = {
                    "Male": "M",
                    "Female": "F",
                    "Prefer not to say": "XNA",
                }[sel]
                self.step += 1
                ...
                if n is None or not (10 <= n <= 99):
                    return (
                        False,
                        "Please enter a two-digit age (18–99)." + PROMPT_SUFFIX_NUMBER,
                    )
                ...
                if not sel:
                    return (
                        False,
                        "Please reply with a number for family status:\n"
                        + _format_options(FAMILY_STATUS_OPTIONS)
                        + PROMPT_SUFFIX_PICK,
                    )
                ...
                if not sel:
                    return (
                        False,
                        "Please reply with 1 (Yes) or 2 (No)." + PROMPT_SUFFIX_PICK,
                    )
                ...
                if n is None or not (1 <= n <= 60):
                    return (
                        False,
                        "Enter a whole number between 1 and 60." + PROMPT_SUFFIX_NUMBER,
                    )
                ...
                if not sel:
                    return (
                        False,
                        "Please reply with a number for organization type:\n"
                        + _format_options(ORG_TYPES)
                        + PROMPT_SUFFIX_PICK,
                    )
                ...
                if n is None or n < 0:
                    return (
                        False,
                        "Enter a positive number of days." + PROMPT_SUFFIX_NUMBER,
                    )
                ...
                if not sel:
                    return (
                        False,
                        "Please reply with a number for education level:\n"
                        + _format_options(EDU_OPTIONS)
                        + PROMPT_SUFFIX_PICK,
                    )
                ...
                if not sel:
                    return (
                        False,
                        "Please reply with a number for income type:\n"
                        + _format_options(INCOME_TYPE_OPTIONS)
                        + PROMPT_SUFFIX_PICK,
                    )
                ...
                if v is None:
                    return (
                        False,
                        "Enter a numeric amount (e.g., 1,500,000)."
                        + PROMPT_SUFFIX_NUMBER,
                    )
                key_map = {
                    11: "AMT_GOODS_PRICE",
